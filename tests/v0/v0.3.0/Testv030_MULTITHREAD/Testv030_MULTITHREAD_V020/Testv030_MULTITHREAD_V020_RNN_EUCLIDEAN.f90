        ...
        subroutine v020RnnEuclidean()
            ...
            do i = 1, 4
                block
                    ...
                    if (size(res) .ne. 2) then; !$OMP CRITICAL; failed = .true.; !$OMP END CRITICAL; return; end if

                    ! empty result: centroid far away
                    res = t%rNN_Centroid([100.0_real64, 100.0_real64], 0.1_real64, metric='euclidean')
                    if (size(res) .ne. 0) then; !$OMP CRITICAL; failed = .true.; !$OMP END CRITICAL; return; end if
