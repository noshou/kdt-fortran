...
```
 ...
 │      └── Testv030_MULTITHREAD/                      	# uses add_kdtest_omp; OMP_NUM_THREADS=4 set per test
 │          ├── Testv030_MULTITHREAD_CONCURRENT_ADD/   	# 4 threads call addNodes concurrently; pop exact; all nodes searchable after (3 metrics)
 │          ├── Testv030_MULTITHREAD_CONCURRENT_RNN/   	# 4 threads call rNN_Centroid concurrently on shared tree (3 metrics)
 ...
 │          ├── Testv030_MULTITHREAD_V020/             	# v0.2.0 search scenarios (single node, empty, duplicates, collinear) run in parallel threads (3 metrics)
 │          ├── Testv030_MULTITHREAD_V021/             	# v0.2.1 lifecycle, isMember, and data scenarios run in parallel threads
 │          ├── Testv030_MULTITHREAD_ADD_NODES/        	# sequential addNodes then 4 concurrent rNN searches (3 metrics)
 │          └── Testv030_MULTITHREAD_INTERNAL_STATE/   	# internal state after concurrent add; independent trees (numMods/ratio/treeId); leaf numMods=4; setRebuildRatio; incremental; rebuild boundary; zero-add
```
